 ...
 *   @note      GT系列电容触摸屏IC通用驱动,本代码支持:
 * GT9147/GT917S/GT968/GT1151/GT9271 等多种 驱动IC, 这些驱动IC仅ID不一样,
 * 具体代码基本不需要做任何修改即可通过本代码直接驱动
 *
...
#define GT9XXX_RST_GPIO_CLK_ENABLE()                                           \
  do {                                                                         \
...
#define GT9XXX_INT_GPIO_CLK_ENABLE()                                           \
  do {                                                                         \
...
#define GT9XXX_RST(x)                                                          \
  do {                                                                         \
    x ? HAL_GPIO_WritePin(GT9XXX_RST_GPIO_PORT, GT9XXX_RST_GPIO_PIN,           \
                          GPIO_PIN_SET)                                        \
      : HAL_GPIO_WritePin(GT9XXX_RST_GPIO_PORT, GT9XXX_RST_GPIO_PIN,           \
                          GPIO_PIN_RESET);                                     \
  } while (0) /* 复位引脚 */

#define GT9XXX_INT                                                                  \
  HAL_GPIO_ReadPin(GT9XXX_INT_GPIO_PORT, GT9XXX_INT_GPIO_PIN) /* 读取做的引脚 \
                                                               */
...
uint8_t gt9xxx_wr_reg(uint16_t reg, uint8_t *buf,
                      uint8_t len); /* 向gt9xx写入数据 */
void gt9xxx_rd_reg(uint16_t reg, uint8_t *buf,
                   uint8_t len);   /* 从gt9xx读取数据 */
