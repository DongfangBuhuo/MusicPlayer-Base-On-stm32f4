...
static void delay_ms(uint16_t ms) { HAL_Delay(ms); }

_m_tp_dev tp_dev = {.init = tp_init,
                    .scan = tp_scan,
                    .adjust = tp_adjust,
                    .x = {0},
                    .y = {0},
...
static uint16_t tp_read_xoy(uint8_t cmd) {
  ...
  for (i = TP_LOST_VAL; i < TP_READ_TIMES - TP_LOST_VAL;
       i++) /* 去掉两端的丢弃值 */
  {
  ...
  if (T_PEN == 0) /* 有按键按下 */
  {
    if (mode) /* 读取物理坐标, 无需转换 */
    {
      tp_read_xy2(&tp_dev.x[0], &tp_dev.y[0]);
                           ...
                           &tp_dev.y[0])) /* 读取屏幕坐标, 需要转换 */
    {
      /* 将X轴 物理坐标转换成逻辑坐标(即对应LCD屏幕上面的X坐标值) */
      tp_dev.x[0] = (signed short)(tp_dev.x[0] - tp_dev.xc) / tp_dev.xfac +
                    lcddev.width / 2;

      /* 将Y轴 物理坐标转换成逻辑坐标(即对应LCD屏幕上面的Y坐标值) */
      tp_dev.y[0] = (signed short)(tp_dev.y[0] - tp_dev.yc) / tp_dev.yfac +
                    lcddev.height / 2;
    }
...
void tp_save_adjust_data(void) {
  ...
  at24cxx_write(TP_SAVE_ADDR_BASE, p,
                12); /* 保存12个字节数据(xfac,yfac,xc,yc) */
...
void tp_adjust(void) {
  ...
  lcd_clear(WHITE); /* 清屏 */
  lcd_show_string(40, 40, 160, 100, 16, TP_REMIND_MSG_TBL,
                  RED);             /* 显示提示信息 */
  ...
  while (1) /* 如果连续10秒钟没有按下,则自动退出 */
  {
    ...
    if ((tp_dev.sta & 0xc000) ==
        TP_CATH_PRES) /* 按键按下了一次(此时按键松开了.) */
    {
      outtime = 0;
      ...
      case 5: /* 全部5个点已经得到 */
        s1 =
            pxy[1][0] - pxy[0][0]; /* 第2个点和第1个点的X轴物理坐标差值(AD值) */
        s3 =
            pxy[3][0] - pxy[2][0]; /* 第4个点和第3个点的X轴物理坐标差值(AD值) */
        s2 =
            pxy[3][1] - pxy[1][1]; /* 第4个点和第2个点的Y轴物理坐标差值(AD值) */
        s4 =
            pxy[2][1] - pxy[0][1]; /* 第3个点和第1个点的Y轴物理坐标差值(AD值) */
        ...
        lcd_show_string(35, 110, lcddev.width, lcddev.height, 16,
                        "Touch Screen Adjust OK!", BLUE); /* 校正完成 */
        delay_ms(1000);
        tp_save_adjust_data();
...
uint8_t tp_init(void) {
  ...
  if (lcddev.id ==
      0x7796) /* 3.5寸屏有两种，一种屏幕ID为0x5510带电阻触摸屏，一种屏幕ID为0x7796带GT型号的电容触摸屏
               */
  {
    if (gt9xxx_init() ==
        0) /* 初始化GT系列触摸屏成功,即当前3.5寸屏为电容触摸屏 */
    {
  ...
  if (lcddev.id == 0X5510 || lcddev.id == 0X9806 || lcddev.id == 0X4342 ||
      lcddev.id == 0X4384 ||
      lcddev.id == 0X1018) /* 电容触摸屏,4.3寸/10.1寸屏 */
  {
    gt9xxx_init();
    tp_dev.scan = gt9xxx_scan; /* 扫描函数指向GT9147触摸屏扫描 */
    tp_dev.touchtype |= 0X80;  /* 电容屏 */
    return 0;
             ...
             lcddev.id ==
                 0X7016) /* SSD1963 7寸屏或者 7寸800*480/1024*600 RGB屏 */
  {
