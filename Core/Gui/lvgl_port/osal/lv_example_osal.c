...
void lv_example_osal(void) {
  ...
  lv_obj_add_event_cb(counter_button, counter_button_event_cb, LV_EVENT_CLICKED,
                      NULL);
  ...
  if (lv_thread_init(&increment_thread, "inc_th", LV_THREAD_PRIO_MID,
                     increment_thread_entry, 2048, NULL) != LV_RESULT_OK) {
...
static void increment_thread_entry(void *user_data) {
  ...
  lv_label_set_text_fmt(counter_label, "Pressed %" LV_PRIu32 " times",
                        press_count);
  lv_unlock();
