...
static int lv_nuttx_uv_timer_init(lv_nuttx_uv_t *uv_info,
                                  lv_nuttx_uv_ctx_t *uv_ctx);
static void lv_nuttx_uv_timer_deinit(lv_nuttx_uv_ctx_t *uv_ctx);

static void lv_nuttx_uv_vsync_poll_cb(uv_poll_t *handle, int status,
                                      int events);
...
static int lv_nuttx_uv_fb_init(lv_nuttx_uv_t *uv_info,
                               lv_nuttx_uv_ctx_t *uv_ctx);
static void lv_nuttx_uv_fb_deinit(lv_nuttx_uv_ctx_t *uv_ctx);

static void lv_nuttx_uv_input_poll_cb(uv_poll_t *handle, int status,
                                      int events);
static int lv_nuttx_uv_input_init(lv_nuttx_uv_t *uv_info,
                                  lv_nuttx_uv_ctx_t *uv_ctx);
...
static void lv_nuttx_uv_disp_poll_cb(uv_poll_t *handle, int status,
                                     int events) {
...
static void lv_nuttx_uv_disp_vsync_request_cb(lv_event_t *e) {
  ...
  if (param) {
    if (fb_ctx->vsync_req_count == 0) {
      LV_LOG_INFO("enabled");
      uv_poll_start(&fb_ctx->vsync_poll, UV_PRIORITIZED,
                    lv_nuttx_uv_vsync_poll_cb);
    }
...
static int lv_nuttx_uv_fb_init(lv_nuttx_uv_t *uv_info,
  ...
  uv_ctx->ref_count++;
  lv_display_add_event_cb(disp, lv_nuttx_uv_disp_vsync_request_cb,
                          LV_EVENT_VSYNC_REQUEST, fb_ctx);

  LV_LOG_USER("lvgl fb loop start OK");

  /* Register for the invalidate area event */
  lv_display_add_event_cb(disp, lv_nuttx_uv_disp_refr_req_cb,
                          LV_EVENT_REFR_REQUEST, fb_ctx);
