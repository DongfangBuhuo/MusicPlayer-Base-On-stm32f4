...
void sccb_init(void)
{
    ...
    /* SCL引脚模式设置,开漏输出,上拉 */
    sys_gpio_set(SCCB_SCL_GPIO_PORT, SCCB_SCL_GPIO_PIN, SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_OD, SYS_GPIO_SPEED_MID,
                 SYS_GPIO_PUPD_PU);

    /* SDA引脚模式设置,开漏输出,上拉, 这样就不用再设置IO方向了, 开漏输出的时候(=1), 也可以读取外部信号的高低电平 */
    sys_gpio_set(SCCB_SDA_GPIO_PORT, SCCB_SDA_GPIO_PIN, SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_OD, SYS_GPIO_SPEED_MID,
                 SYS_GPIO_PUPD_PU);
